...
public partial class ParallaxSystem : MonoBehaviour
{
    ...
    private void Update()
    {
        Vector2 movement = new Vector2(-_speed.x * _direction.x, -_speed.y * _direction.y);//движение игрока
        ...
        if (_isLooping) {
            ...
            if (firstChild != null) {
                if (firstChild.Transform.position.y > _mainCamera.transform.position.y) {
